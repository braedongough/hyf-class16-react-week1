  ...
  function UserItemExpanded({fullname, address, age, height, spokenLanguages}) {
    ...
    return (
      <>
          ...
          <h3>Spoken Languages: 
              <ul>
                {spokenLanguages.map((language) => <li>{language}</li>)}
              </ul>
          </h3>
      </>
    )
  ...
  function UserList ({users})  {
    ...
    return (
        <ul>
            {
              users.map(user => {
                return <UserItemExpanded fullname={user.fullname} address={user.address} age={user.age} height={user.height} spokenLanguages={user.languages}/>;
              })
            }
        </ul>
    );
